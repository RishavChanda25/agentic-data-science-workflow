...
def supervisor_node(state: DataScienceState):
    """The Baseline Supervisor with Latency Tracking."""
    print("🤖 [Supervisor] Analyzing state to determine next step...")
    ...
    try:
        ...
        next_route = result.next_node
        print(f"🤖 [Supervisor] Proposed route: {next_route}")
        
        return {
            "next_node": next_route,
            "total_sleep_time": current_sleep + sleep_duration  # <-- LOG THE DELAY
        }
        ...
        return {
            ...
            "next_node": "FINISH",
            "total_sleep_time": current_sleep + sleep_duration  # <-- LOG IT EVEN ON ERROR
        }